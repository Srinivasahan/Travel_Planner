...
const EnquiryForm = () => {
  ...
  const handlePayment = () => {
    const amount = formData.packageAmount;
    ...
    const options = {
      key: 'rzp_test_4NzpA1bW3pp3IS', // Your Razorpay key
      key_secret: 'a5xsHAtVmefoFY9SPUhixmwz', // Your Razorpay secret (you might not need this in client-side code)
  ...
  const handlePaymentSuccess = async (paymentId) => {
    try {
      // Send payment ID to your server to confirm payment and store it
      await axios.post('http://localhost:8080/api/payment', { paymentId, ...formData });
      setSubmitted(true);
    } catch (error) {
